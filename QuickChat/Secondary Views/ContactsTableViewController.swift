...
class ContactsTableViewController: UITableViewController, UISearchResultsUpdating, UserTableViewCellDelegate {
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if !isGroup {
            ...
            if checkBlockStatus(withUser: userToChat) {
               ...
               ProgressHUD.show("This user is not available for chat")
            }
        } else {
            //group chat
    ...
    @objc func nextBtnPressed() {
        let newGroupVC = UIStoryboard.init(name: "Main", bundle: nil).instantiateViewController(withIdentifier: "nextGroupView") as! NewGroupViewController
    ...
    func setupButtons() {
        if isGroup {
            //for group chat
            let nextBtn = UIBarButtonItem(title: "Next", style:.plain, target: self, action: #selector(self.nextBtnPressed))
            ...
            //for one on one chat
            let inviteBtn = UIBarButtonItem(image: UIImage(named: "invite"), style: .plain, target: self, action: #selector(self.inviteBtnPressed))
            
            let searchBtn = UIBarButtonItem(image: UIImage(named: "nearMe"), style: .plain, target: self, action: #selector(self.searchNearByBtnPressed))
